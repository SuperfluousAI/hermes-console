...
import type { HermesUsageSummary, UsageBreakdownRow, UsageSessionRecord, UsageWindowId, UsageWindowSummary } from "@/features/usage/types";
...
function toUsageRecord(agentId: string, agentLabel: string, session: ReturnType<typeof readStateDbSessions>[number]): UsageSessionRecord {
  return {
    id: session.id,
    agentId,
    agentLabel,
...
export function readHermesUsage(now = new Date()): HermesUsageSummary {
  ...
  const records = agents.flatMap((agent) =>
    readStateDbSessions(agent.rootPath).map((session) => toUsageRecord(agent.id, agent.label, session)),
  );
