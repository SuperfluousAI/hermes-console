...
function DiagnosticsSummary({ diagnostics }: { diagnostics: DiagnosticsResponse['data'] }) {
  const summaryItems = [
    {
      label: 'doctor issues',
    ...
    },
    {
      ...
      value:
        diagnostics.status.scheduledJobs.active == null
          ? '—'
  ...
  return (
    <section className="rounded-lg border border-border bg-surface/70 p-4">
      ...
      <p className="mt-3 text-sm leading-7 text-fg-muted">
        ...
        rest of the app keeps working if those commands are unavailable.
      </p>
      ...
      {issues.length > 0 ? (
        <div className="mt-4 space-y-3">
          {issues.map((issue) => (
            <article key={issue.id} className="rounded-md border border-amber-500/30 bg-amber-500/10 p-3">
