...
export const MemoryPage = () => {
  ...
  const summaryItems = [
    {
      label: 'status',
      value: memory.status === 'ready' ? '✓ OK' : memory.status === 'partial' ? '~ Partial' : 'Missing',
      detail:
        memory.status === 'ready'
    ...
    <div className="space-y-8">
      ...
      <section className="max-w-3xl">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <RefreshButton loadedAt={query.data.meta.capturedAt} queryKeys={[apiQueryKeys.memory]} />
        </div>
        ...
        <p className="mt-3 text-sm leading-7 text-fg-muted">
          Hermes stores durable memory in plain markdown files. This page shows current usage against configured limits
          and the saved blocks parsed out of those files.
        </p>
      </section>
