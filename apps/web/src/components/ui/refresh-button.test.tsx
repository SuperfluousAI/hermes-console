...
describe('RefreshButton', () => {
  it('refetches only the explicit query keys it is given', async () => {
    ...
    const refetchQueriesSpy = vi.spyOn(queryClient, 'refetchQueries').mockResolvedValue(undefined);

    render(
      <QueryClientProvider client={queryClient}>
        <RefreshButton loadedAt="2026-04-06T12:00:00.000Z" queryKeys={[['files'], ['file-content', 'agents-md']]} />
      </QueryClientProvider>
    );
