...
export function QueryStatusCard({
  ...
  return (
    <section className={['rounded-lg border p-4', containerClass(status)].join(' ')}>
      <div className="flex flex-wrap items-center gap-2">
        <p className="font-[family-name:var(--font-bricolage)] text-base font-semibold text-fg-strong">{title}</p>
      ...
      <div className="mt-4 space-y-3">
        {issues.map((issue) => (
          <article key={issue.id} className={['rounded-md border p-3', issueClass(issue.severity)].join(' ')}>
