...
describe('isPathInsideDirectory', () => {
  ...
  it('returns 404 for missing asset-like requests instead of serving the SPA index', async () => {
    const webDistDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hermes-console-static-'));
    fs.writeFileSync(path.join(webDistDir, 'index.html'), '<!doctype html><html><body>index</body></html>');
  ...
  it('returns a controlled text response when index.html exists but is unreadable', async () => {
    ...
    expect(await response.text()).toBe('Hermes Console web assets could not be read.');
  });
