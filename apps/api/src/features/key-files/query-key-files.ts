...
export function readKeyFilesDataQuery(): HermesQueryResult<KeyFilesData> {
  ...
  const issues: HermesQueryIssue[] = [...installation.issues, ...keyFiles.issues, ...memory.issues];
  ...
  if (!installation.data.hermesRootExists) {
    issues.push({
      ...
      detail: 'Hermes-root files cannot be discovered because the configured Hermes root does not exist.',
  ...
  if (keyFiles.data.files.length === 0 && installation.data.hermesRootExists) {
    issues.push({
      ...
      detail: 'Hermes Console did not find any of its known Hermes-root or workspace instruction files.'
    });
  }
  ...
  return createHermesQueryResult({
    ...
    capturedAt,
    status: !installation.data.hermesRootExists ? 'missing' : keyFiles.data.files.length === 0 ? 'partial' : 'ready',
    issues
  });
}

export function readKeyFileContentQuery({ fileId }: { fileId: string }): HermesQueryResult<KeyFileContentData> | null {
  ...
  if (selectedFile.content == null) {
    issues.push({
      ...
      detail: 'The selected file exists but could not be read as UTF-8 text from the local filesystem.',
