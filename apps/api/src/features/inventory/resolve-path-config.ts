...
function expandHomeDirectoryPrefix({ homeDir, value }: { homeDir: string; value: string }) {
  if (value === '~') {
    return homeDir;
  }
...
export function normalizeConfiguredPath({ homeDir, value }: { homeDir: string; value: string }) {
...
export function resolveInventoryPathConfig(config: InventoryPathConfig): InventoryPathResolution {
...
export function isWorkspaceRootConfigured(paths: InventoryPathResolution) {
