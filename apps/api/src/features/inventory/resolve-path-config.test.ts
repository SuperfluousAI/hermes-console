...
import { normalizeConfiguredPath, resolveInventoryPathConfig } from '@/features/inventory/resolve-path-config';
...
describe('resolveInventoryPathConfig', () => {
  ...
  it('uses expanded env overrides for both hermes and workspace roots', () => {
    ...
    expect(paths.workspaceRoot.path).toBe(path.resolve('/home/tester/workspace/project'));
  });
});
