...
export function readHermesInstallationResult() {
  ...
  const profilesRootPath = path.join(paths.hermesRoot.path, 'profiles');
    ...
    return createReadResult({
      ...
      issues: [
        createUnreadablePathIssue({
          ...
          detail:
            error instanceof Error ? error.message : 'Hermes Console could not inspect the local Hermes installation.',
