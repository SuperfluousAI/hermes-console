...
export function readHermesSkillsQuery(): HermesQueryResult<SkillsIndexResult> {
  ...
  if (!installation.data.hermesRootExists) {
    issues.push({
      ...
      detail: 'Installed skills cannot be discovered because the configured Hermes root does not exist.',
  ...
  if (agentsWithSkills.length === 0) {
    issues.push({
      ...
      detail: 'Hermes Console did not find any skills directories under the detected agent roots.',
