...
export function readHermesCronQuery(): HermesQueryResult<HermesCronIndex> {
  ...
  if (!installation.data.hermesRootExists) {
    issues.push({
      ...
      detail: 'Scheduled job data cannot be read because the configured Hermes root does not exist.',
  ...
  if (agentsWithCron.length === 0) {
    issues.push({
      ...
      detail: 'Hermes Console did not find cron directories under the detected agent roots.',
